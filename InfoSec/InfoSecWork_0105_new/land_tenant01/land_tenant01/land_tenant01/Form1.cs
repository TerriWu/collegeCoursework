...
namespace land_tenant01
{
    public partial class Form1 : Form
    {
        ...
        string land_signdata, land_publickey;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (radioButton1.Checked) //房東
            {
                ...
                send_to("rigister:"+textBox1.Text+":"+textBox2.Text,"127.0.0.1",3000);
                ...
                while (rigister_ft == true)
                {
                    ...
                    //私鑰存入電腦中
                    StreamWriter sw = new StreamWriter("D:/private.txt");
                    //StreamWriter sw = new StreamWriter("c:/private.txt");
                    sw.Write(rsa.ToXmlString(true));
                    sw.Close();
                    //公鑰存入電腦中
                    StreamWriter swP = new StreamWriter("D:/public.txt");
                    //StreamWriter sw = new StreamWriter("c:/private.txt");
                    swP.Write(rsa.ToXmlString(false));
                    swP.Close();
            ...
            if (udp.Available > 0)
            {
                ...
                switch (token[0])
                {
                    case "land_to_tenant_sign:":
                        ...
                        string land_menomy = token[3], land_address = token[4], land_date = token[5];
                        textBox9.Text = land_menomy; textBox7.Text = land_address; textBox6.Text = land_date;
                    ...
                    case "tenant_insert_ok":
                        label15.Text = token[1];
                        label15.Visible = true;
                        break;

                    case "land_select_reult":
        ...
        {
            //簽章確認，假如沒有填資料，label顯示提示字，否則.......???
            if (textBox3.Text == "" || textBox4.Text == "" || textBox5.Text == "")
            {
                label10.Text = "訊息提示：未填寫租金";
                label10.Visible = true;

                label11.Text = "訊息提示：未填寫地址";
                label11.Visible = true;

                label12.Text = "訊息提示：未填寫時間";
            ...
            StreamReader sr = new StreamReader("D:/private.txt");
            rsa.FromXmlString(sr.ReadLine());
            sr.Close();

            byte[] msg = Encoding.UTF8.GetBytes(textBox3.Text + textBox4.Text + textBox5.Text);
            byte[] signature = rsa.SignData(msg, "SHA1");
            //String signature_str = Convert.ToBase64String(signature);

            //signature_str傳伺服器再做房客簽章
            send_to("land_sign:" + Convert.ToBase64String(signature) + ":" + textBox3.Text + ":" + textBox4.Text + ":" + textBox5.Text + ":" + textBox1.Text, "127.0.0.1", 3000);
        }

        private void button6_Click(object sender, EventArgs e)//查詢
        {
            send_to("land_select_con:"+textBox1.Text, "127.0.0.1", 3000);
        }
        ...
        private void button8_Click(object sender, EventArgs e)//房客簽章確認
        {
